...
export class ContactService {
  ...
  async create(user: User, request: CreateContactRequest): Promise<ContactResponse> {
    this.logger.info(`UserService.update(${JSON.stringify(user.username)} - ${JSON.stringify(request)})`);
    const validatedRequest: CreateContactRequest = this.validationService.validate(
  ...
  async update(user: User, request: UpdateContactRequest): Promise<ContactResponse> {
    const validatedRequest: UpdateContactRequest = this.validationService.validate(
      ContactValidation.UPDATE,
      request
    );

    const contact = await this.checkContactMustExists(user.username, validatedRequest.id);
  ...
  async search(user: User, request: SearchContactRequest): Promise<WebResponse<ContactResponse[]>> {
    const validatedRequest = this.validationService.validate(ContactValidation.SEARCH, request);
