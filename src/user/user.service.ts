...
export class UserService {
  ...
  async register(request: RegisterUserRequest): Promise<UserResponse> {
    this.logger.info(`UserService.register(${JSON.stringify(request.username)})`);

    const validatedRequest: RegisterUserRequest = this.validationService.validate(UserValidation.REGISTER, request);
  ...
  async login(request: LoginUserRequest): Promise<UserResponse> {
    ...
    const validatedRequest: LoginUserRequest = this.validationService.validate(UserValidation.LOGIN, request);
  ...
  async update(user: User, request: UpdateUserRequest): Promise<UserResponse> {
    ...
    const validatedRequest: UpdateUserRequest = this.validationService.validate(UserValidation.UPDATE, request);
